...
points(tAll@data$Dist[tAll@data$Trans == 2], tAll@data$DGM[tAll@data$Trans == 2], pch = 23, cex = (tAll@data$PNT_LIVE/50+.5), col = alpha(tAll@data$Vcol, .8), lwd =900)
...
x11()
plot(tAll@data$Dist, tAll@data$DGM, xlim = c(125, 350), typ = "n", ylim = c(880, 884), ylab = "Höhe ü. NN. [m]", xlab = "Distanz zur Flussmitte")
lines(pT2$dist, ((pT2$Z+pT3$Z+pT4$Z+pT5$Z)/4))
lines(density(tAll@data$Dist))
